<div wire:key="{{$item->id}}" drag-item="{{$item->id}}" id="{{$item->id}}" 
class="w-full {{ $item->lead_prefix ? 'bg-stone-300 dark:bg-stone-800' : 'bg-white dark:bg-gray-950' }} mb-2 text-left rounded-md cursor-pointer border border-gray-200 drop-shadow-lg">
    <div class="p-2">
        @if($editingTodoId === $item->id)
            <div>
                
                <input 
                    type="date" name="editingTodoDate" wire:model='editingTodoDate' 
                ...
                <input 
                    ...
                    class="border border-gray-400 text-gray-900 text-sm rounded-md block w-full p-2 mt-2"
                >
                <textarea name="editingTodoNote" wire:model='editingTodoNote' placeholder="{{__('todo.add_note')}}..."
    ...
    @endif
    <div class="flex justify-end bg-slate-500 rounded-b-md p-2 dark:bg-slate-800">
        @if ($item->lead_prefix)
        <a href="#" target="_blank" title="{{__('todo.see_lead')}}">
