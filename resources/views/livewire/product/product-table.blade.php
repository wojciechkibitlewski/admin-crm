...
<div class="bg-white rounded-xl p-4 dark:bg-gray-800 ">
    ...
    <table class="text-left text-sm font-normal mb-4 w-full">
        ...
        dark:bg-gray-600 dark:text-gray-100 dark:border-neutral-500">
            <tr>
                <th scope="col" class="px-2 py-2 font-light">{{__('products.name')}}</th>
                <th scope="col" class="px-2 py-2 font-light">{{__('products.price')}}</th>
                <th scope="col" class="px-2 py-2 font-light">{{__('products.category')}}</th>
        ...
        <tbody>
            @foreach ($products as $item)
                ...
                hover:bg-gray-200 dark:border-neutral-500 dark:hover:bg-neutral-600">
                    ...
                    <td class="whitespace-nowrap px-2 py-2">{{ $item->price }} zł</td>
                    <td class="whitespace-nowrap px-2 py-2">{{ Helper::getCategoryName($item->category_id) }}</td>
                    <td class="whitespace-nowrap px-2 py-2 flex flex-row">
                        <div class="p-1">
                            ...
                            href="{{ route('products.show',$item->id) }}">{{__('products.show')}}</a>
                        </div>
